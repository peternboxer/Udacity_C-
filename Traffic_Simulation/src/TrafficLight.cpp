...
T MessageQueue<T>::receive()
{
    ...
    /* only wait while queue is empty */
    /* stop_waiting once !_queue.empty() == true */
    _condition.wait(lk, [&]{return !_queue.empty();});
    T recved = _queue.front();
    _queue.pop_front();
    return recved;
}
...
void MessageQueue<T>::send(T &&msg)
{
    ...
    _queue.clear();     // clear old traffic light msgs
...
void TrafficLight::waitForGreen()
{
    ...
    while(true){
        if(_TLqueue.receive() == TrafficLightPhase::green){
            return;
        }
    }
}
...
void TrafficLight::cycleThroughPhases()
{
    ...
    uint8_t waitDuration = std::rand()%3 + 4;
